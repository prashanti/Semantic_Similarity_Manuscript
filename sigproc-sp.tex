...
\subsubsection{Semantic similarity metrics}
The following similarity metrics were shortlisted for a comparison of performance based on distinguishing real similarity from noise:
\begin{itemize}
\item Resnik IC \cite{pesquita2009semantic} : The similarity of two terms is the IC of their MICA.
\item Lin IC \cite{pesquita2009semantic} : 2 times the IC of the MICA divided by the sum of the two term ICs. Includes a relative measure of how far the terms are from the MICA. Lies in range [0,1]
\item Schlicker IC \cite{pesquita2009semantic}: Modification of Lin IC. Multiplies Lin IC with (1- p(MICA)). Uses the probability of the MICA as a weight. Terms with more informative MICAs are considered more similar.
\item Couto IC \cite{pesquita2009semantic}: Similarity of two terms is the mean IC of all LCAs. Uses all LCAs instead of just the most informative ancestor.
\item Jaccard Index \cite{pesquita2009semantic}: Ratio of the number of common ancestors to all number of all ancestors 
\item Hybrid Relative Specificity Similarity \cite{wu2013improving}: A hybrid measure that uses IC and edge distance. Uses distance between terms to the MICA, IC of the MICA, and IC difference between the terms and the most informative leaf subsumed by the term (Figure \ref{hrss}).
\item BOQA: Uses bayesian networks to model semantic similarity queries and takes into account the probability of valid annotations missing in the query (true positives) and unrelated annotations being included in the query (false positives). 
\end{itemize}
 \begin{figure}
 \centering
 \includegraphics[width=1\textwidth]{hrss.jpg}
 \caption{\label{hrss} Calculation of Hybrid Relative Specificity Similarity \cite{wu2013improving} } \end{figure}
